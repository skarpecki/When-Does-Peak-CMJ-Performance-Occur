...
7) **Select session paths**: the statistics run for two session-selection paths: best mRSI day (`best_mrsi_day`) and typical mRSI day (`median_mrsi_day`, closest to each athlete’s median session-average mRSI). Session selections are written to `data/<selection_path>_session_per_athlete.csv`; the legacy `data/best_session_per_athlete.csv` is still written for the best-mRSI path.
8) **Outlier handling and plots**: for each metric in `METRICS_CONFIG`, rows beyond ±4 SD in the z-score are dropped, best reps per athlete are chosen within each selected session path, and `plot_raincloud` produces PNG + CSV + stats per metric. Each path writes plots under `data/plots/<selection_path>/<metric>/...`. Summary CSVs land in `data/all_summary.csv`, path-specific `data/<selection_path>_all_summary.csv`, and `data/outliers_summary.csv`.
