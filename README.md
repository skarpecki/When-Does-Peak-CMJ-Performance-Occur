...
8) **Outlier handling and plots**: for each metric in `METRICS_CONFIG`, rows beyond ±4 SD in the z-score are dropped, best reps per athlete are chosen, and `plot_raincloud` produces PNG + CSV + stats per metric (e.g., `data/plots/jump_height_m/CMJ_jump_height_m_raincloud.png`). Summary CSVs land in `dataall_summary.csv` and `data/outliers_summary.csv`. Panel images are assembled under `data/plots/raincloud_panels/`.
9) **Chi-square + G-Test (LRT) effect size**: for each metric’s best-rep distribution across trials, a chi-square and G-test (LRT) goodness-of-fit test (df=5 for six reps) plus Bonferroni-adjusted p-value and Cramer's V are computed. Per-metric results are saved to `data/plots/<metric>/CMJ_<metric>_chi2.csv`, aggregated across metrics in `data/chi2_g_summary.csv`, and echoed to `data/pipeline_stats.log`.
